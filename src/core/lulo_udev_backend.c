...
static int spawn_lulod(LuloUdevBackend *backend)
{
    char path[PATH_MAX];
    pid_t pid;
    int devnull = -1;
...
static int request_via_lulod(LuloUdevBackend *backend, int full, const LuloUdevState *state,
                             LuloUdevSnapshot *snap, char *err, size_t errlen)
{
    char socket_path[108];
    uint32_t type = full ? LULOD_REQ_UDEV_FULL : LULOD_REQ_UDEV_ACTIVE;
    int fd = -1;
    ...
    if (lulod_send_udev_request(fd, type, state) < 0) {
        if (err && errlen > 0) snprintf(err, errlen, "failed to send request to lulod");
        close(fd);
        return -1;
    }
    if (lulod_recv_udev_response(fd, snap, err, errlen) < 0) {
...
static void *lulo_udev_backend_main(void *opaque)
{
    LuloUdevBackend *backend = opaque;

    for (;;) {
        int do_full = 0;
        int do_active = 0;
        LuloUdevState state = {0};
        LuloUdevSnapshot work = {0};
        char err[160] = {0};
        int rc = 0;
        ...
        if (do_full || do_active) rc = request_via_lulod(backend, do_full, &state, &work, err, sizeof(err));
        ...
        if (rc < 0) {
            ...
            lulo_udev_snapshot_free(&work);
        } else {
            backend->error[0] = '\0';
            lulo_udev_snapshot_free(&backend->published);
...
int lulo_udev_backend_start(LuloUdevBackend *backend)
{
    ...
    backend->running = 1;
    if (pthread_create(&backend->tid, NULL, lulo_udev_backend_main, backend) != 0) {
        backend->running = 0;
...
static void lulo_udev_backend_request(LuloUdevBackend *backend, const LuloUdevState *state,
                                      int full, int active)
{
    if (!backend || !backend->started) return;
                              ...
                              unsigned *generation, LuloUdevBackendStatus *status)
{
    ...
    if (dst && generation && backend->have_snapshot && *generation != backend->generation) {
        rc = lulo_udev_snapshot_clone(&fresh, &backend->published);
        if (rc == 0) {
