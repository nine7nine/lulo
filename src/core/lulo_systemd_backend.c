...
static int spawn_lulod(LuloSystemdBackend *backend)
{
    char path[PATH_MAX];
    pid_t pid;
    int devnull = -1;
...
static int request_via_lulod(LuloSystemdBackend *backend, int full, const LuloSystemdState *state,
                             LuloSystemdSnapshot *snap, char *err, size_t errlen)
{
    char socket_path[108];
    uint32_t type = full ? LULOD_REQ_SYSTEMD_FULL : LULOD_REQ_SYSTEMD_ACTIVE;
    int fd = -1;
    ...
    if (lulod_send_systemd_request(fd, type, state) < 0) {
        if (err && errlen > 0) snprintf(err, errlen, "failed to send request to lulod");
        close(fd);
        return -1;
    }
    if (lulod_recv_systemd_response(fd, snap, err, errlen) < 0) {
...
static void *lulo_systemd_backend_main(void *opaque)
{
    LuloSystemdBackend *backend = opaque;

    for (;;) {
        int do_full = 0;
        int do_active = 0;
        LuloSystemdState state = {0};
        LuloSystemdSnapshot work = {0};
        char err[160] = {0};
        int rc = 0;
        ...
        if (rc < 0) {
            ...
            lulo_systemd_snapshot_free(&work);
        } else {
            backend->error[0] = '\0';
            lulo_systemd_snapshot_free(&backend->published);
...
int lulo_systemd_backend_start(LuloSystemdBackend *backend)
{
    ...
    backend->running = 1;
    if (pthread_create(&backend->tid, NULL, lulo_systemd_backend_main, backend) != 0) {
        backend->running = 0;
...
static void lulo_systemd_backend_request(LuloSystemdBackend *backend, const LuloSystemdState *state,
                                         int full, int active)
{
    if (!backend || !backend->started) return;
                                 ...
                                 unsigned *generation, LuloSystemdBackendStatus *status)
{
    ...
    if (dst && generation && backend->have_snapshot && *generation != backend->generation) {
        rc = lulo_systemd_snapshot_clone(&fresh, &backend->published);
        if (rc == 0) {
    ...
    lulo_systemd_snapshot_free(&fresh);
