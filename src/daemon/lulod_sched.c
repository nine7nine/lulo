                                     ...
                                     char *err, size_t errlen)
{
    ...
    if (lulod_system_send_sched_apply_preset_request(fd, preset_id) < 0) {
        if (err && errlen > 0) snprintf(err, errlen, "failed to send scheduler preset request");
        close(fd);
        return -1;
    }
    if (lulod_system_recv_status_response(fd, err, errlen) < 0) {
        if (err && errlen > 0 && !err[0]) {
            snprintf(err, errlen, "failed to read scheduler preset response");
        }
        close(fd);
        return -1;
    }
    close(fd);
    return lulod_sched_snapshot_gather(snap, 0, err, errlen);
}
